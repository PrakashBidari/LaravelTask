...
class NewsController extends Controller
{
            ...
            if ($request->expectsJson()) {
                try {
                    ...
                    ';
                        ...
                        ->editColumn('isBlog', function ($news) {
                            return $news->isBlog === 1 ? 'True' : 'False';
                        })
    ...
    public function blog(Request $request)
    {
        try {
            if ($request->expectsJson()) {
                try {
                    $allNews = News::where('isBlog','1')->latest();
                    return DataTables::of($allNews)
                        ->addIndexColumn()
                        ->addColumn('image', function ($row) {
                            if ($latestImage = $row->image) {
    ...
    public function update(AddNewsRequest $request, News $news)
    {
        try {
            $validated_data = $request->validated();
            $this->newsService->update($validated_data, $news);
            toastify()->success('News Updated Successful!');
